    ...
    formatter: function (params) {
      ...
      for (var i = 0; i < mapobj.length; i++) {
        if (params.name == mapobj[i].name) {
          str = params.name +
            '</br>移网：' + parseFloat(mapobj[i].ywval).toFixed(2) +
            '</br>宽带：' + parseFloat(mapobj[i].kdval).toFixed(2) +
            '</br>'
        }
      }
      return str
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      for (var i = 0; i < result.proN.length; i++) {
        jdataN.push({
          name: result.proN[i].PROVINCE_NAME,
          // 注意百分比
          value: result.proN[i].FINISH_PLAN*100
        })
      }
      for (var i = 0; i < result.proS.length; i++) {
        jdataS.push({
          name: result.proS[i].PROVINCE_NAME,
          value: result.proS[i].FINISH_PLAN*100
        })
      }
      jdata = jdataN.concat(jdataS);
      for (var i = 0; i < jdata.length; i++) {
        if (jdata[i].value >= 100) {
          jdata[i].value = 100
        } else if (jdata[i].value < 100 && jdata[i].value >= 50) {
          jdata[i].value = 50
        } else if (jdata[i].value < 50) {
          jdata[i].value = 0
        }
      }
      console.log(jdata);
  ...
  success: function (result) {
    ...
    for (var i = 0; i < result.province.length; i++) {
      mapobj.push({
        ...
        // 同比
        same: result.province[i].MONTH_TO_MONTH,
        // 环比
        circle: result.province[i].YEAR_TO_YEAR
      })
    }
